...
export default function OrderDetail() {
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      const [orderRes, offersRes, messagesRes] = await Promise.all([
    ...
    <Layout>
      ...
      <section className="mb-6">
        ...
        {offers.map(o => (
          <div key={o.id} className="border p-4 mb-2">
            ...
            {order.buyer_id === order.id && order.status === 'open' && (
        ...
        {order.status === 'open' && (
          <div className="mt-4 p-4 border">
            ...
            <input
              ...
              onChange={e => setNewOffer(v => ({ ...v, price: Number(e.target.value) }))}
              className="mb-2 p-2 border w-full"
            />
            <input
              type="number" placeholder="Срок (дни)" value={newOffer.delivery_time}
              onChange={e => setNewOffer(v => ({ ...v, delivery_time: Number(e.target.value) }))}
              className="mb-2 p-2 border w-full"
            />
            <textarea
      ...
      {order.status === 'completed' && (
        <section>
          ...
          <select value={review.rating} onChange={e => setReview(v => ({ ...v, rating: Number(e.target.value) }))} className="mb-2 p-2 border">
            {[1, 2, 3, 4, 5].map(n => <option key={n} value={n}>{n} звёзд</option>)}
          </select>
          <textarea value={review.comment} onChange={e => setReview(v => ({ ...v, comment: e.target.value }))} className="mb-2 p-2 border w-full" placeholder="Комментарий" />
